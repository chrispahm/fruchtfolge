...
function parseElanXml (xmlString, callback) {
	...
	Promise.all(json.nn.land.parzelle.map(function (field) {
		if (ignoredCodes.indexOf(field.nutzungaj.code) > -1) {
		    ...
		    // See usage codes in according variable for info
		  	...
		  	fieldObject = {};
		  	// Add field no	
		    fieldObject.fieldid = id;

		  	// Add field name
		  	if (typeof field.schlag.bezeichnung !== 'undefined') {
		  		fieldObject.name = field.schlag.bezeichnung
		    }
		    else {
		    	// if name unknow add 'no name'
		    	fieldObject.name = "Ohne Bezeichnung";
		    }
		    // Add field size
		    fieldObject.size = parseFloat((field.nettoflaeche / 10000).toFixed(2))

		    // Declare as root crop capable
		    fieldObject.rootCrop = true

		    // Add previous crops
	    	// For year n - 1
	    	// if a previous crop is given for that year
	        if (field.nutzungvj.code !== undefined) {
	            fieldObject[n-1] = usageCodes[usageCodes.indexOf(field.nutzungvj.code) + 1];
	        }
	        // For year n
	        fieldObject[n] = usageCodes[usageCodes.indexOf(field.nutzungaj.code) + 1];

	        // sync with DB
	        	return firebase.database().ref(userPath + '/fields').child(id).update(fieldObject);
			} 			
		})).then(function () {
		    if (callback) {
		    	return callback()
