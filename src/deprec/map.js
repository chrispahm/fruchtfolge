...
map.on('load', function () {
  ...
  // Draw fields on map
  firebase.database().ref('users/' + firebase.auth().currentUser.uid + '/fields/').once('value').then(function(snapshot) {
    if (snapshot.val()) {
      Object.keys(snapshot.val()).forEach(function (field) {
        Draw.add(snapshot.val()[field].polygon)
      });
    }
  });
